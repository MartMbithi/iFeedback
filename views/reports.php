...
/* Watermark - Use Gomc Logo */
$watermark_path = 'https://web.pharmacyboardkenya.org/wp-content/uploads/2023/09/new-ppb-logo.png';
$watermark_type = pathinfo($watermark_path, PATHINFO_EXTENSION);
$watermark_data = file_get_contents($watermark_path);
$watermark = 'data:image/' . $watermark_type . ';base64,' . base64_encode($watermark_data);
...
/* Compliments */
if (isset($_POST['Export_Compliments'])) {
    ...
    if ($department == 'All') {
        if ($file_type == 'PDF') {
            /* Load PDF Report */
            require_once('../reports/pdf/all_departments_compliments.php');
        } else {
            /* Load CSV Report */
            require_once('../reports/csv/all_departments_compliments.php');
        }
    } else {
        if ($file_type == 'PDF') {
            /* Load PDF Report */
